...
** programming functions
   #+BEGIN_SRC emacs-lisp
     ...
     (defun non-google3-cpp-coding-style/setup ()
       (let ((filename (buffer-file-name)))
         ;; Enable kernel mode for the appropriate files
         (when (and filename
                    (string-match (expand-file-name "~/kernel")
           ...
           (unbind-key "\C-t" helm-gtags-mode-map)
           (define-key c-mode-map (kbd "C-t") 'ff-find-related-file)
           (message "Setting up indentation for non google c++"))))
...
** multi-term
   this is not for vterm
   #+BEGIN_SRC  emacs-lisp
   ;;  (use-package multi-term
   ;;    :ensure t
   ;;    :bind (("C-c t c" . multi-term)
   ;;           ("C-c t r" . rename-buffer)
   ;;           ("C-c t t" . term-mode-toggle))
   ;;    :config
...
** vterm
   #+BEGIN_SRC emacs-lisp
     ...
     (use-package vterm-toggle
       :ensure t
       :init
       (bind-key "C-c s x" 'vterm-toggle)
       )
     (use-package multi-vterm
       :ensure t
       :init
       (bind-key "C-c s c" 'multi-vterm)
       (bind-key "C-c s t" 'multi-vterm-dedicated-toggle)
       (bind-key "C-c s n" 'multi-vterm-next)
       )
   #+END_SRC
...
** programming hooks
   #+BEGIN_SRC emacs-lisp
     ...
     (add-hook 'c++-mode-hook 'non-google3-cpp-coding-style/setup)
